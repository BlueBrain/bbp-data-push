...
def initialize_pusher_cli(
    ...
    if nexus_env in default_environments:
        ...
        L.error(
            f"Error: {nexus_env} do not correspond to one of the 3 possible environment: "
            "dev, staging, prod"
        )
        exit(1)
...
def push_volumetric(ctx, dataset_path, voxels_resolution, config, provenances):
    ...
    dataset = create_volumetric_resources(
        ctx.obj["forge"], dataset_path, voxels_resolution, config, provenances, ctx.obj["verbose"]
    )
    if dataset:
        _push_to_Nexus(
            dataset, ctx.obj["forge"], "https://neuroshapes.org/dash/volumetricdatalayer"
        )
...
def push_meshes(ctx, dataset_path, config, hierarchy_path, provenances):
    ...
    dataset = create_mesh_resources(
        ctx.obj["forge"], dataset_path, config, hierarchy_path, provenances, ctx.obj["verbose"]
    )
    if dataset:
        _push_to_Nexus(
            dataset, ctx.obj["forge"], "https://neuroshapes.org/dash/brainparcellationmesh"
        )
...
def push_cellrecords(ctx, dataset_path, voxels_resolution, config, provenances):
    ...
    if dataset:
        _push_to_Nexus(dataset, ctx.obj["forge"], "https://neuroshapes.org/dash/cellrecordseries")
