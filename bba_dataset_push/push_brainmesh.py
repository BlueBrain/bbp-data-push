...
from bba_dataset_push.commons import (get_brain_region_name, get_hierarchy_file,
                                      add_contribution, append_provenance_to_description)
from bba_dataset_push.logging import create_log_handler

L = create_log_handler(__name__, "./push_meshes.log")

def create_mesh_resources(forge, inputpath: list, config_path, input_hierarchy: list, 
    ...
    for filepath in inputpath:
        ...
        if os.path.isdir(filepath):
            ...
            try:
                ...
                elif os.path.samefile(filepath, mesh_path["brain_region_meshes_l23split"]):
                    hierarchy_tag = "hierarchy_l23split"
                else:
                    L.error(f"Error: The '{filepath}' folder do not correspond to one of the "\
                            "brain meshes folder dataset defined in the MeshFile section of the "\
            ...
            L.error(f"Error: '{filepath}' is not a directory. The input dataset need to be a "\
                    "directory containing OBJ brain meshes")
            exit(1)
        ...
        try:
            ...
            L.error(f"ValueError in {'meshpath'} file name. {error}. The mesh file names have "\
                    "to be integer representing their region")
            exit(1)
        ...
        try:
            ...
            region_name, hierarchy_tree = get_brain_region_name(region_id, hierarchy_path, flat_tree)
        ...
        if isinstance(forge._store, DemoStore):
            ...
            try:
                mesh_resource.contribution, log_info = add_contribution(forge, mesh_resource)
                L.info('\n'.join(log_info))
        ...
        for f in range(1,len(files_mesh)): #start at the 2nd file
            ...
            try:
                ...
                L.error(f"ValueError in {'meshpath'} file name. {error}. The mesh file names "\
                        "have to be integer representing their region")
                exit(1)
            try:
                region_name, hierarchy_tree = get_brain_region_name(region_id, hierarchy_path,
                                                                 flat_tree)
            except KeyError as e:
                L.error(f"KeyError: {e}")
                exit(1)
