...
def test_create_mesh_resources():
    ...
    wrong_dataset_name = str(Path(TEST_PATH, "tests/tests_data/wrong_data/wrong_mesh_dataset_name"))
    not_a_dir = str(Path(TEST_PATH, "tests/tests_data/wrong_data/brain_region_meshes_hybrid/1.obj"))
    ...
    config_data_emptyhierarchy = str(
        Path(
            TEST_PATH,
            "tests/tests_data/wrong_data/wrong_data_config_file/emptyHierarchy_push_dataset_config.yaml",
        )
    )
    config_data_wronghierarchy = str(
        Path(
            TEST_PATH,
            "tests/tests_data/wrong_data/wrong_data_config_file/wrongHierarchy_push_dataset_config.yaml",
        )
    )
    ...
    mesh_resource_fulloptions[
        "description"
    ] = "Brain region mesh - Region_1 (ID: 1). It is based in the parcellation volume resulting of the hybridation between CCFv2 and CCFv3. Generated in the Atlas Pipeline by the module 'parcellation2mesh' version 0.0.1."
