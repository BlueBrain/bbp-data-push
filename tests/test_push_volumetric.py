...
def test_create_volumetric_resources():
    ...
    dataset_path = [str(Path(TEST_PATH, 'tests/tests_data/annotation_hybrid.nrrd')),
    ...
    empty_folder = str(Path(TEST_PATH,
                            'tests/tests_data/wrong_data/empty_cell_density'))
    wrong_dataset_name = str(Path(TEST_PATH,
                            'tests/tests_data/wrong_data/wrong_name.nrrd'))
    corrupted_data_header = str(Path(TEST_PATH,
                            'tests/tests_data/wrong_data/corrupted_header.nrrd'))
    ...
    neuron_density_file = str(Path(TEST_PATH,
                              'tests/tests_data/wrong_data/neuron_density_file.nrrd'))
    ...
    config_corruptedData = str(Path(TEST_PATH, 'tests/tests_data/wrong_data/wrong_data_config_file/corruptedData_push_dataset_config.yaml'))

    wrong_provenance = "wrong_provenance"

    volumetric_dict_simple = volumetric_dict(cell_density = False, nrrd_props = True)

    result = vars(create_volumetric_resources(forge, [dataset_path[0]], voxel_resolution, 
                                       config_path, provenances =[None], verbose = 0)[-1])
    ...
    cell_density_dict_fulloptions['description'] = "Inhibitory neuron density volume for the Hybrid annotation volume from ccfv2 and ccfv3 at 25 microns. Generated in the Atlas Pipeline by the module 'atlas-building-tools cell-densities inhibitory-neuron-densities' version 1.0.0."
    
    result = vars(create_volumetric_resources(forge, dataset_path, voxel_resolution, config_path, 
    ...
    with pytest.raises(KeyError) as e:
        create_volumetric_resources(forge, [dataset_path[0]], voxel_resolution, wrong_config_key,
                                  provenances = [provenance], verbose = 0)[-1]
    assert str(e.value) == "'annotation_hybrid'"

    # dataset is an empty folder
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [empty_folder], voxel_resolution, 
                                  config_data_emptydata, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1

    # dataset with wrong name
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [wrong_dataset_name], voxel_resolution,
                                  config_path, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1

    # configuration file contains not existing file path
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [dataset_path[0]], voxel_resolution,
                                  config_data_notfound, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1

    # annotation dataset is a folder
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [folder_annotation], voxel_resolution,
                                  config_wrongdatatype, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1

    # cell density dataset is a file
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [neuron_density_file], voxel_resolution,
                                  config_wrongdatatype, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1
    ...
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [corrupted_data_header], voxel_resolution,
                                  config_corruptedData, provenances = [provenance],
                                  verbose = 0)[-1]
    assert e.value.code == 1
    
    # provenance is wrong
    with pytest.raises(SystemExit) as e:
        create_volumetric_resources(forge, [dataset_path[0]], voxel_resolution, config_path,
...
def test_add_nrrd_props():
    ...
    nrrd_header = {
        ...
        'space directions': np.array([[25.,  0.,  0.], [ 0., 25.,  0.], [ 0.,  0., 25.]]),
    ...
    result = vars(add_nrrd_props(volumetric_resource, nrrd_header, config,
                                correct_voxel_type))
