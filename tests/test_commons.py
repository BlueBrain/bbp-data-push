...
from bba_dataset_push.commons import (get_voxel_type, append_provenance_to_description, 
                                      get_hierarchy_file, get_brain_region_name,
                                      add_contribution)
...
def test_append_provenance_to_description():
    ...
    assert append_provenance_to_description(provenance, module_tag) == "Generated in the "\
    ...
    assert "The provided provenance string argument must be of the form '<module_name>:"\
    ...
    with pytest.raises(ValueError) as e:
        append_provenance_to_description(provenance, module_tag)
    assert "Input 'provenance' string 'module:version 1' does not contain the right module "\
           "name." in str(e.value)
...
def test_get_hierarchy_file():
    ...
    assert (get_hierarchy_file(input_hierarchy, config_content, hierarchy_tag) == 
...
def test_get_brain_region_name():
    ...
    assert get_brain_region_name(region_id, hierarchy_path, flat_tree)[0] == "Basic cell "\
           "groups and regions"
